# LedgerAI Client - AI 智能记账 App# LedgerAI Client



<div align="center">一个规范的 React Native 项目，包含完整的目录结构和可复用组件示例。
...
[![Status](https://img.shields.io/badge/Status-In%20Development-red.svg)]()- 🧩 **可复用组件** - Button、Input、Card 等通用组件

- 📱 **完整示例页面** - 包含表单、列表、卡片等常用功能

**基于 React Native 的跨平台 AI 智能记账移动应用**- 📝 **TypeScript 支持** - 类型安全，更好的开发体验
...
LedgerAI Client 是一款**创新的 AI 驱动智能记账应用**，将传统记账与人工智能完美结合。用户可以通过**自然语言对话**完成记账、查询、分析等操作，极大简化了记账流程，让理财变得更加轻松和智能。
...
- 🤖 **AI 对话记账**: 通过自然语言与 AI 对话完成记账，无需复杂操作- **[项目结构说明](./PROJECT_STRUCTURE.md)** - 详细的目录和组件说明

- 💬 **实时智能助手**: 基于 WebSocket 的实时对话，流式响应体验

- 📊 **可视化统计**: 直观的收支统计、趋势分析## 📁 项目结构
...
- 🎨 **现代UI设计**: 参考 Telegram、Google 等优秀应用的设计理念├── screens/           # 页面组件

- 🔐 **安全可靠**: JWT 认证，数据加密传输│   └── HomeScreen.tsx # 主页示例（包含表单、列表、卡片等）

├── components/        # 组件目录

## 🎬 产品截图│   └── common/       # 通用组件（Button, Input, Card）
...
- @react-navigation/stack 7.5.0<Button 

- Context API (全局状态)  title="点击我" 

  onPress={() => alert('Hello!')}

**UI 组件**:  variant="primary"  // primary | secondary | outline | text

- react-native-gifted-chat 2.8.1 (聊天界面)  size="medium"      // small | medium | large

- react-native-calendars 1.1313.0 (日历选择)/>
...
**开发工具**:  placeholder="请输入用户名"

- ESLint (代码规范)  value={username}

- Jest (单元测试)  onChangeText={setUsername}
...
│  │          Presentation Layer           │  │  <Text>卡片内容</Text>
...
│  │            Data Layer                 │  │To start the Metro dev server, run the following command from the root of your React Native project:

│  │  ┌──────────┐  ┌──────────────────┐  │  │

│  │  │   API    │  │  Local Storage   │  │  │```sh
